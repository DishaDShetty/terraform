...
instance_types=$(cat infracost_output.yml | yq eval '.[] | select(.name | test("AWS EC2")) | .resources[].values.instance_type' -)
...
threshold=3
cost=$(cat infracost_output.yml | yq eval '.[] | select(.name | test("AWS EC2")) | .monthlyCost.value' -)
